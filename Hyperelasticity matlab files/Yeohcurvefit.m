% Yeji Han
% Curvefit for Yeoh model parameters -- Uniaxial tension, compression, and planar shear test
...
uni_abaqus = table2array(readtable("BBDINO_Abaqus_Tension_Yeoh.csv"));
...
plan_abaqus = table2array(readtable("BBDINO_Abaqus_PlanarShear_Yeoh.csv"));
...
comp_abaqus = table2array(readtable("BBDINO_Abaqus_Compression_Yeoh.csv"));
...
% Initial guess for [C1, C2, C3]
% params0 = [0.1, 0.1 0.1];  % [C1, C2, C3]
% params0 = [0.5 0.5 0.5]
params0 = [1 1 1];
...
params_fit = lsqnonlin(@(p) cost_fun_Yeoh_all(p, data, weights), params0, [], [], options);
C1 = params_fit(1); 
C2 = params_fit(2);
C3 = params_fit(3);
...
sigma_model = yeoh_model(params_fit, lambda_model, 'uniaxial');
...
saveas(gcf, fullfile(outdir, '1.model_fit_comparison_uniaxial_Yeoh.png'));
...
sigma_model = yeoh_model(params_fit, lambda_model, 'planar');
plot(eps_model, sigma_model, 'k-', 'LineWidth', 2.5);
...
lambda_model = exp(eps_model);
sigma_model = yeoh_model(params_fit, lambda_model, 'compression');
plot(eps_model, sigma_model, 'k-', 'LineWidth', 2.5);
...
% sigma_true_model = yeoh_model(params_fit, lambda_model, 'uniaxial');
...
% legend({'sample 1','sample 2','sample 3','Yeoh model'},'FontSize',15,'Location','best');
% set(gca,'FontSize',15,'FontName','Arial','LineWidth',2);
% grid off; box on;
% set(gcf,'Position',[400 50 800 600]);
% saveas(gcf, fullfile(outdir,'4.engineering_uniaxial_Yeoh.png'));
% 
% % ==== Planar shear ====
% figure();
...
% lambda_model = 1 + eps_model_eng;
% sigma_true_model = yeoh_model(params_fit, lambda_model, 'planar');
% sigma_eng_model = sigma_true_model ./ lambda_model;
% plot(eps_model_eng, sigma_eng_model, 'k-', 'LineWidth', 2.5);
% 
% xlabel('Engineering Strain', 'FontName','Arial','FontSize',15);
...
% grid off; box on;
% set(gcf,'Position',[400 50 800 600]);
% saveas(gcf, fullfile(outdir,'5.engineering_planarshear_Yeoh.png'));
% 
% % ==== Compression ====
% figure();
...
% lambda_model = 1 + eps_model_eng;
% sigma_true_model = yeoh_model(params_fit, lambda_model, 'compression');
% sigma_eng_model = sigma_true_model ./ lambda_model;
% plot(eps_model_eng, sigma_eng_model, 'k-', 'LineWidth', 2.5);
% 
% xlabel('Engineering Strain', 'FontName','Arial','FontSize',15);
...
% grid off; box on;
% set(gcf,'Position',[400 50 800 600]);
% saveas(gcf, fullfile(outdir,'6.engineering_compression_Yeoh.png'));
...
%% ======================= COST FUNCTION =============================
function E_all = cost_fun_Yeoh_all(params, data, weights)
    modes = {'uni', 'plan', 'comp'};
    E_all = [];
    ...
    for i = 1:3
        ...
        for k = 1:length(data.(mode))
            ...
            sigma_model = yeoh_model(params, lambda, mode);
...
%% ======================= YEOH MODEL =======================
function sigma = yeoh_model(params, lambda, mode)
    C1 = params(1);
    C2 = params(2);
    C3 = params(3);
    ...
    switch lower(mode)
        case {'uni','uniaxial'}
            ...
            sigma = 2*(lambda.^2 - lambda.^(-1)) .* ...
                    (C1 + 2*C2.*(I1 - 3) + 3*C3.*(I1 - 3).^2);

        case {'plan','planar'}
            I1 = lambda.^2 + lambda.^(-2) + 1;
            sigma = 2*(lambda.^2 - lambda.^(-2)) .* ...
                    (C1 + 2*C2.*(I1 - 3) + 3*C3.*(I1 - 3).^2);
