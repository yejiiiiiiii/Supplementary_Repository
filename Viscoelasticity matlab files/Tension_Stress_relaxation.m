...
%% Constants
epsilon0 = 1;  % Constant strain during test
% orders = [2, 3, 5, 9];  % Orders of Prony Series to loop over
orders = [2 5];  % Orders of Prony Series to loop over

area = 6*2; % mm^2, sample cross-sectional area

%% Load data
% time, strain, stress (kPa)
d1 = table2array(readtable('BBDINO30_TensionRelaxation (1).csv'));
d2 = table2array(readtable('BBDINO30_TensionRelaxation (2).csv'));
d3 = table2array(readtable('BBDINO30_TensionRelaxation (3).csv'));
...
saveas(gcf, fullfile(outdir, '2. tensionrelaxation_avg.png'));
saveas(gcf, fullfile(outdir,'2. tensionrelaxation_avg.fig'))
...
for idx = 1:length(orders)
    ...
    figure;
    % subplot(1,2,1)
    plot(t_avg, Et_avg, 'r-','Linewidth',2, 'DisplayName', 'Data');
    hold on;
    plot(t_avg, prony(p_fit, t_avg), 'b--', 'LineWidth', 2, 'DisplayName', 'Prony Fit');
    hold off

    xlabel('Time [s]','FontName', 'Times New Roman', 'FontSize', 25)
    ylabel('Tensile stress [kPa]','FontName', 'Times New Roman', 'FontSize', 25);
    ...
    filename = sprintf('%d.tensionrelaxation_Prony_%d.png', iter, N);
    filename2 = sprintf('%d.tensionrelaxation_Prony_%d.fig', iter, N);
